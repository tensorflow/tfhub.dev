# Module google/wiki40b-lm-lt/1
Embeddings and negative log likelihood from a language model
trained on Lithuanian from Wiki40B dataset with sentence-piece
vocabulary size 32k.
...
This specific module is trained on Lithuanian available in Wiki40B dataset.
It is trained using a 12 layers Transformer XL model [2]. The input to the
module is variable length text in Lithuanian, and the output is a 768
...
g = tf.Graph()
with g.as_default():
  module = hub.Module("https://tfhub.dev/google/wiki40b-lm-lt/1")
