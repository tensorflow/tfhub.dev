# Tfjs rishit-dagli/faster-rccn-cppe5-tfjs/fp16/tfjs/1
TFJS FP16 quantized version of the Faster RCNN model (with ResNet 101 backbone) trained on the CPPE - 5 (Medical Personal Protective Equipment) dataset [1].
...
```js
const model = await tf.loadGraphModel("https://tfhub.dev/rishit-dagli/faster-rccn-cppe5-tfjs/fp16/tfjs/1")
```
