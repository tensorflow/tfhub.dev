...
*   `prediction:logits`: A vector of 99410 similarity scores, corresponding to
    ...
    landmark may appear in the labelmap multiple times; there are 4411 unique
    labels. Because the output vectors contains duplicate labels, **users of
    this model should postprocess its output**. Remove duplicates by keeping the
    highest-scoring logit for each unique label.
