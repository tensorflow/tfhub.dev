# Module google/edgetpu/nlp/mobilebert-edgetpu/m/1
...
The medium version (m) pretrainer model contains 50.9M parameters, achieves
**73.8% Masked LM accuracy**. Using it as the backbone for SQUAD v1.1, we acheve
**89.5% accuracy with 15.1ms latency** on Tensor SoC Edge TPU. As a comparison,
...
computing. The unquantized model shows **90.2% accuracy with 24.8ms latency**.
...
encoder_network = hub.KerasLayer(
    'https://tfhub.dev/google/edgetpu/nlp/mobilebert-edgetpu/m/1',
    trainable=True)
