# Module google/on_device_vision/classifier/landmarks_classifier_africa_V1/1
...
*   `prediction:logits`: A vector of 19293 similarity scores, corresponding to
    items in the
    ...
    in English, for example, "Mount Kilimanjaro National Park". Each landmark
    may appear in the labelmap multiple times; there are 2779 unique labels.
    Because the output vectors contains duplicate labels, **users of this model
    should postprocess its output**. To do this, group each dimension of the
    output vector by the corresponding labels in the label map, and take the
