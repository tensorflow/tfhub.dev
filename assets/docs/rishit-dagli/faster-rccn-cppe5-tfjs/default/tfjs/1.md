# Tfjs rishit-dagli/faster-rccn-cppe5-tfjs/default/tfjs/1
TFJS version of the Faster RCNN model (with ResNet 101 backbone) trained on the CPPE - 5 (Medical Personal Protective Equipment) dataset [1].
...
```js
const model = await tf.loadGraphModel("https://tfhub.dev/rishit-dagli/faster-rccn-cppe5/default/tfjs/1")
```
