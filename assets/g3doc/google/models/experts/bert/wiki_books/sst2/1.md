# Module google/&zwnj;experts/&zwnj;bert/&zwnj;wiki_books/&zwnj;sst2/1

BERT trained on Wikipedia and BooksCorpus and fine-tuned on SST-2.
...
This model uses a BERT base architecture[1] initialized from https://tfhub.dev/google/experts/bert/wiki_books/1 and fine-tuned on SST-2[5] and was exported from code in the [TensorFlow Official Model Garden](https://github.com/tensorflow/models/tree/master/official/nlp/bert).
...
This model was initialized from the https://tfhub.dev/google/experts/bert/wiki_books/1 checkpoint and then fine-tuned on SST-2 for 3 epochs with a batch size of 32 and
...
# Load the BERT model
bert = hub.load('https://tfhub.dev/google/experts/bert/wiki_books/sst2/1')
...
This model is fine-tuned on the SST-2 and is recommended for use in sentiment analysis tasks.

The fine-tuning task uses the Stanford Sentiment Treebank (SST-2) dataset to predict the sentiment in a given sentence. This model may not perform well on tasks unrelated to sentiment analysis.
...
[5]: [Stanford Sentiment Treebank (SST-2) dataset](https://nlp.stanford.edu/sentiment/index.html)
