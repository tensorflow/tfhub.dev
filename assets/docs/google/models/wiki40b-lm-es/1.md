# Module google/wiki40b-lm-es/1
Embeddings and negative log likelihood from a language model
trained on Spanish from Wiki40B dataset with sentence-piece
vocabulary size 32k.
...
This specific module is trained on Spanish available in Wiki40B dataset.
It is trained using a 12 layers Transformer XL model [2]. The input to the
module is variable length text in Spanish, and the output is a 768
